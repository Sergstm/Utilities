...
public class HardService implements Service {
    ...
    public BigDecimal getSum() {
        ...
        if (volumeCounter.getVolume().compareTo(new BigDecimal("100")) < 1) {
            result = volumeCounter.getVolume().multiply(tariff1);
        } else {
            result = new BigDecimal("100").multiply(tariff1);
            BigDecimal reminder = volumeCounter.getVolume().subtract(new BigDecimal("100"));
